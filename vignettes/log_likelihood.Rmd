...
To put this all together, we will calculate the likelihood (i.e. predicted value using the logistic function), and the log-likelihood (what we maximized). The plot shows the observed values (as triangles), predicted values (as squares), and the log-likelihood (as circles). When we take the log of one minus the likelihood the values we get values less than zero, hence the need to maximize those values instead of minimizing.
