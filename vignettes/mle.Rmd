...
Note that we have to estimate a third parameter, sigma, which is the error term and defines the standard deviation for the normal distribution for estimating the likelihood. This is connected to the distirubtion of the residuals as we will see later. We can now calculate the log-likelihood for any combination of parameters.
...
Where $i$ is the observation, $x_i$ is the observed value, $\hat{x_i}$ is the estimated (predicted) value, and $N$ is the sample size. Below, we see that the numerical optizer matches the RMSD within a rounding error.
